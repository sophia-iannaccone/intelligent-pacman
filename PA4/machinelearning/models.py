...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        for i, x in enumerate(xs[1:]):
            ...
            lin_hid = nn.Linear(z, self.w_hidden)
            z = nn.Add(lin, lin_hid)
    ...
    def train(self, dataset):
        ...
        while acc < 0.86:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                self.w.update(grad[0], -self.learning_rate)
                self.w_hidden.update(grad[1], -self.learning_rate)
                self.w_final.update(grad[2], -self.learning_rate)
            acc = dataset.get_validation_accuracy()